...
PROJECT_ROOT = REPO_ROOT / "examples" / "projects" / "oracle_analysis_like"
if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))
if str(REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(REPO_ROOT))

from open_predictive_coder import ByteCodec, TrainModeConfig  # noqa: E402
from model import OracleAnalysisLikeConfig, OracleAnalysisLikeModel  # noqa: E402


class OracleAnalysisLikeTests(unittest.TestCase):
    def test_analyze_uses_train_mode_checkpoints_and_route_coverage(self) -> None:
        config = OracleAnalysisLikeConfig()
        model = OracleAnalysisLikeModel(config)
    ...
    def test_detached_mode_still_analyzes_but_changes_state_usage(self) -> None:
        detached = OracleAnalysisLikeConfig(
        ...
        model = OracleAnalysisLikeModel(detached)
